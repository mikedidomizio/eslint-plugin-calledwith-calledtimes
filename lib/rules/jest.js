						...
						if (numberNthCalledWith === 1) {
							...
							if (!isOrdered) {
								context.report({
									node: offendingNode,
									fix(fixer) {
										// we go all the way back, then traverse forward until we hit `toHaveBeenCalledTimes`
										let firstNthCalledWith = false;
										let curNode = node;

										while (!firstNthCalledWith) {
											const previousNode = getPreviousNode(curNode);
											const previousNodePropertyName = getNodePropertyName(previousNode);

											if (previousNodePropertyName !== toHaveBeenNthCalledWith) {
												firstNthCalledWith = true;
											} else {
										...
										while (!inOrder) {
											const curNodeNthCalledWith = getNumberOfNthCalledWith(curNode);
											const nextNode = getNextNode(curNode);
											const nextNodeNthCalledWith = getNumberOfNthCalledWith(nextNode);

											if (curNodeNthCalledWith > nextNodeNthCalledWith) {
												const nodeText = context.sourceCode.getText(curNode);
												const nextNodeText = context.sourceCode.getText(nextNode);
												console.log(nodeText, nextNodeText);

												// removes the first node and put the second node in its place
												fixes.push(fixer.remove(curNode), fixer.insertTextAfter(curNode, nextNodeText));
