				...
				if (nodeName === 'toHaveBeenCalledWith' || nodeName === 'toHaveBeenNthCalledWith') {
					...
					if (!nextNode || getNodePropertyName(nextNode) !== 'toHaveBeenCalledTimes') {
						...
						const customReportMessage = {
							message: nodeName === 'toHaveBeenNthCalledWith' ? context.options[0]?.toHaveBeenNthCalledWith?.reportMessage : context.options[0]?.toHaveBeenCalledWith?.reportMessage
						}

						context.report({
							node,
							...(customReportMessage.message ? customReportMessage : generalMessages),
						});
					}
				}
			},
