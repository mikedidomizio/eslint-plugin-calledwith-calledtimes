				...
				if (nodeName === toHaveBeenCalledTimes) {
					if (!previousNode) {
						...
						if (nodeIdentifier !== prevNodeIdentifier) {
							const calledWithMatcherBefore = [toHaveBeenCalledWith, toHaveBeenNthCalledWith].includes(getNodePropertyName(previousNode))

							if (calledWithMatcherBefore) {
								context.report({
									node,
								...
								context.report({
									node,
									fix(fixer) {
										// proceeds to move toHaveBeenCalledTimes to the very end
										const fixes = []

										// todo need to move toHaveBeenCalledTimes to the very end
