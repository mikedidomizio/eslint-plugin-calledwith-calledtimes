const { getErrorMessageObject, getIdentifier, getNodePropertyName, getNextNode, getPreviousNode } = require('./helpers');
...
module.exports = {
	meta: {
		...
		messages: {
            ...
            orderedNthCalledList: 'Please put them in the correct call order that it was called',
				...
				if (nodeName === toHaveBeenCalledTimes) {
                    if (!previousNode) {
                        ...
                        if (nodeIdentifier !== prevNodeIdentifier) {
                            context.report({
                                node,
                                ...getErrorMessageObject(node, context.options, 'identifiersAreNotMatching'),
                            });
                        }
                        return
                    }
